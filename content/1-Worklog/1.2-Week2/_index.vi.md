...
| 4   | - **Thực hành:** <br>&emsp; + Tạo EC2 instance với Security Group tùy chỉnh (lần đầu cấu hình thiếu inbound rule cho port 22 nên không SSH được, phải rà soát lại rule mới kết nối được) <br>&emsp; + Kết nối SSH bằng key pair <br>&emsp; + Gắn thêm EBS volume, mount và resize volume mà không cần tắt máy | 22/04/2026   | 22/04/2026      | <https://cloudjourney.awsstudygroup.com/vi/> |
...
* Xây dựng được VPC tùy chỉnh với subnet public/private trải trên 2 AZ, cấu hình route table đúng phạm vi (sau khi tự phát hiện và sửa lỗi gắn nhầm route ra Internet Gateway cho subnet private), và hiểu rõ sự khác biệt thực tế giữa Security Group (stateful) và Network ACL (stateless).
* Rút ra kinh nghiệm là nên kiểm tra kỹ inbound rule và route table trước khi thực hành, tránh mất thời gian debug kết nối như đã gặp trong tuần.
* ...
