...
| Wed | - **Practice:** <br>&emsp; + Launch an EC2 instance with a custom Security Group (first attempt was missing an inbound rule for port 22, couldn't SSH in until the rule was reviewed and fixed) <br>&emsp; + Connect via SSH using a key pair <br>&emsp; + Attach and mount an additional EBS volume, then resize it without downtime | 22/04/2026 | 22/04/2026      | <https://cloudjourney.awsstudygroup.com/vi/> |
...
| Fri | - **Practice:** <br>&emsp; + Build a custom VPC from scratch with public/private subnets across two Availability Zones <br>&emsp; + Configure route tables so only the public subnet can reach the Internet Gateway (first attempt mistakenly attached the 0.0.0.0/0 route to the private subnet's route table, had to check and split the route tables correctly) <br>&emsp; + Configure a least-privilege Security Group for EC2 (only required ports open) | 24/04/2026 | 24/04/2026      | <https://cloudjourney.awsstudygroup.com/vi/> |
...
* Built a custom VPC with public/private subnets across two AZs, correctly scoped route tables (after catching and fixing a mistake where the private subnet's route table was wrongly pointed at the Internet Gateway), and understood the practical difference between Security Groups (stateful) and Network ACLs (stateless).
* Learned to double-check inbound rules and route tables before starting a lab, to avoid the connectivity debugging time lost this week.
* ...
